...
def make_gaussian_log_prior(weight_decay, temperature):
    ...
    def log_prior_diff(params1, params2):
        ...
        diff = sum([(p1**2 - p2**2).sum() for p1, p2 in zip(params1, params2)])
...
def preprocess_network_outputs_gaussian(predictions):
    ...
    predictions_mean, predictions_std = torch.split(
        predictions, [num_dims, num_dims], dim=-1
    )
...
def make_gaussian_log_likelihood(temperature):
    def gaussian_log_likelihood(model, x, y, batch_size=None):
        ...
        if batch_size is not None:
            ...
            y = y[indices]
        predictions = model(x)
        predictions_mean, predictions_std = preprocess_network_outputs_gaussian(
            predictions
        )
        tempered_std = torch.clamp_min(predictions_std * np.sqrt(temperature), 1e-10)
        se = (predictions_mean - y) ** 2
        log_likelihood = -0.5 * se / tempered_std**2 - 0.5 * torch.log(
            tempered_std**2 * 2 * math.pi
        )
...
def get_best_hyperparameters(train_x, train_y, llh_fn):
    ...
    for prior_var in prior_var_grid:
        for noise_var in noise_var_grid:
            noise_var_tensor = torch.tensor(
                noise_var, dtype=train_x.dtype, device=train_x.device
            )
    ...
    return best_prior_var, torch.tensor(
        best_noise_var, dtype=train_x.dtype, device=train_x.device
    )
            ...
            if i < len(self.dimensions) - 2:
                if activation == "tanh":
                    ...
                    raise NotImplementedError(
                        "Activation type %s is not supported" % activation
                    )
